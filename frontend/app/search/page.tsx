...
function SearchContent() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get("/search", { params:{q} }).then(r => setResults(r.data)).finally(() => setLoading(false));
  }, [q]);
      ...
      <div className="page-header">
        <div className="max-w-7xl mx-auto">
          ...
          <form onSubmit={e => { e.preventDefault(); setQ(input); router.push(`/search?q=${encodeURIComponent(input)}`); }} className="flex gap-2 max-w-xl">
            <input value={input} onChange={e => setInput(e.target.value)} placeholder="Search events, businesses, destinations…" autoFocus className="input-base flex-1" />
